...
mysql_select_db($db);
if ($_POST['kk2']!="") {
    $query_s = "SELECT	no_mutasi FROM 	pergerakan_stok a LEFT JOIN detail_pergerakan_stok b ON b.id_stok = a.id
WHERE 	a.typestatus = '1' AND b.nokk LIKE '$_POST[kk2]%'
GROUP BY 	no_mutasi ORDER BY 	no_mutasi DESC";
} elseif ($_POST['no']!="") {
    $query_s = "SELECT no_mutasi FROM mutasi_kain where  no_mutasi !=''  and no_mutasi like '$_POST[no]%'  $t group by no_mutasi order by no_mutasi desc";
} else {
    $query_s = "SELECT no_mutasi FROM mutasi_kain where  no_mutasi !=''  and no_mutasi like '$_POST[no]%'  $t group by no_mutasi order by no_mutasi desc";
}

$query_limit_s = sprintf("%s LIMIT %d, %d", $query_s, $startRow_s, $maxRows_s);
$s = mysql_query($query_limit_s) or die(mysql_error());
...
?>
    ...
    <form id="form1" name="form1" method="post" action="">
      <table width="517">
        <tr>
          <td colspan="3">
            <div align="center">CEK LAPORAN MUTASI</div>
            </div>
            <?php
    ...
    <table width="61%" border="5">
      <tr class="hurufs">
        <td bgcolor="#0000FF">
          <div align="center">No</div>
        </td>
        <td bgcolor="#0000FF">
          <div align="center">No Mutasi</div>
        </td>
        <td bgcolor="#0000FF">
   ...
   $no=1; do {
      ...
      <tr>
        ...
        <td>
          <?php echo $row_s[no_mutasi]; ?>
        </td>
        <td align="center"> <a href="pages/cetak/cetak_mutasi_ulang.php?mutasi=<?php echo $row_s[no_mutasi]; ?>" target="_blank">Cetak</a> | | <a href="pages/mutasi_lihat.php?mutasi=<?php echo $row_s[no_mutasi]; ?>" target="_blank">Lihat</a> | | <a
            href="pages/lihat_cek.php?mutasi=<?php echo $row_s[no_mutasi]; ?>" target="_blank">Cek</a> || <a href="pages/cetak/cetak_mutasi_ulang_bs.php?mutasi=<?php echo $row_s[no_mutasi]; ?>" target="_blank">Waste</a> || <a href="pages/cetak/cetak_mutasi_ulang_stok.php?mutasi=<?php echo $row_s[no_mutasi]; ?>" target="_blank">Stok</a></td>
      </tr>
      <?php $no++;
    ...
    <table border="0">
      <tr>
        <td>
          <?php if ($pageNum_s > 0) { // Show if not first page?>
        ...
        <td>
          <?php if ($pageNum_s < $totalPages_s) { // Show if not last page?>
