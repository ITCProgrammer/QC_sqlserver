...
$db_name="TM";
 set_time_limit(600);
    ...
    if($_GET['bagian']=="1"){$bgn="not";$bagian1="PACKING";}else{$bgn=" ";$bagian1="INSPEK MEJA";}
...
if(yrd=="yard"){
	...
	return false;
       ...
       <div align="center"><b>BUKTI MUTASI KAIN DETAIL ONLINE <?php echo $bagian1;?></b> </div><div><b>Tanggal : <?php echo $_GET['tgl1']." s/d ".$_GET['tgl2']; ?> <br>
...
where stockmovement.wid=22 and stockmovement.transactiontype=4 and stockmovement.transactionstatus=1
 and CAST(TM.dbo.stockmovement.[note] AS VARCHAR(8000)) $bgn like '%IM%'
and productprop.batchno='$_GET[id]' Order by productprop.rollno asc");
  ...
  ?><div>
      ...
      </form></div><table width="100%" align="center">
 ...
 $yardd1=round(1000/(($row2['Width']*$row2['WeightPerArea'])/43.05),2);
    ...
    <td align="right"><?php echo number_format($mtr1,'2','.',',');?></td>
     <td><?php echo $row2['refno'];?></td>
    </tr>
      <?php
	   if($row2['refno']=="SISA" ||$row2['refno']=="FOC"){$netto=0; $ukuran=0;}else{$netto=$row2['weight']; $ukuran=$mtr1;}
