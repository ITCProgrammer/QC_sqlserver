...
$Data = $initialized;
$Data .= $condensed1;
$jumlah=mysql_query("SELECT *,tmp_detail_kite.yard_ as yard_, tmp_detail_kite.yard_ as roll, tmp_detail_kite.net_wight as bruto,sisa FROM tbl_kite left join tmp_detail_kite on tmp_detail_kite.nokkKite=tbl_kite.nokk
WHERE tbl_kite.nokk='$_GET[id]'
ORDER BY tbl_kite.id ASC");
$jml=mysql_num_rows($jumlah);
$row4=mysql_num_rows($jumlah);
$sql=mysql_query("SELECT *,tmp_detail_kite.yard_ as yard_, tmp_detail_kite.yard_ as roll, tmp_detail_kite.net_wight as bruto,sisa 
...
ORDER BY tbl_kite.id ASC limit 0,20"); 
 $x=0;
  ...
  {
	  $sql1=mysql_query("SELECT *,tmp_detail_kite.yard_ as yard_, tmp_detail_kite.yard_ as roll, tmp_detail_kite.net_wight as bruto,sisa 
FROM tbl_kite left join tmp_detail_kite on tmp_detail_kite.nokkKite=tbl_kite.nokk
WHERE tbl_kite.nokk='$_GET[id]'
...
$sql2=mysql_query("SELECT *,tmp_detail_kite.yard_ as yard_, tmp_detail_kite.yard_ as roll, tmp_detail_kite.net_wight as bruto,sisa 
FROM tbl_kite left join tmp_detail_kite on tmp_detail_kite.nokkKite=tbl_kite.nokk
WHERE tbl_kite.nokk='$_GET[id]'
...
//$Data .="\n";
  ...
  if( $row['sisa']=="TH" || $row['sisa']=="SISA" ||$row['sisa']=="FOC"){$netto=0; $ukuran=0;$rol=0;}else{$rol=1;$netto=$row['bruto']; $ukuran=$row['yard_'];}
	 $totbruto=$totbruto+ $netto;
	  $totyard=$totyard+ $ukuran;
	  $totrol=$totrol+$rol;
$x++;
}
$Data .="|TOTAL :        ".$totrol." X ".$totbruto." | ".$totyard." |";
//echo $Data;
fwrite($handle, $Data);
fclose($handle);
