...
$sql=mysqli_query($con," SELECT * FROM tmp_detail_kite 
WHERE  nokkkite = '".$_GET['id']."'  
 $ket  AND (sisa!='TH' and sisa!='FKTH' and sisa!='BS' and sisa!='BB') group by no_roll
ORDER BY no_roll ASC limit 0,20"); 
$sql2=mysqli_query($con,"SELECT * FROM tmp_detail_kite 
WHERE  nokkkite = '".$_GET['id']."'  
 $ket AND (sisa!='TH' and sisa!='FKTH' and sisa!='BS' and sisa!='BB') group by no_roll
ORDER BY no_roll ASC limit 20,20"); 
$sql3=mysqli_query($con,"SELECT * FROM tmp_detail_kite 
WHERE  nokkkite = '".$_GET['id']."'  
 $ket AND (sisa!='TH' and sisa!='FKTH' and sisa!='BS' and sisa!='BB') group by no_roll
ORDER BY no_roll ASC limit 40,20");
 ...
 <?php
  ...
  {	
    ...
    <td align="right"><?php echo number_format($row['net_wight'],'2','.',','); ?></td>
    <td align="right"><?php echo number_format($row['yard_'],'2','.',',');?></td>
    <td align="center"><?php echo $row['grade'];?></td>
    <td><?php echo $row['rolexp'];?></td>
     <td><?php echo $row['sisa'];?></td>
    </tr>
      <?php
	   if($row['sisa']=="SISA" ||$row['sisa']=="EXTRA"){$netto=0; $ukuran=0;}else{$netto=$row['net_wight']; $ukuran=$row['yard_'];}
  ...
  {	
    ...
    <tr bgcolor="#9999FF">
      ...
      <td align="right"><?php echo number_format($row2['net_wight'],'2','.',','); ?></td>
    ...
    <?php
	 if($row2['sisa']=="SISA" ||$row2['sisa']=="EXTRA"){$netto=0; $ukuran=0;}else{$netto=$row2['net_wight']; $ukuran=$row2['yard_'];}
	 $totbruto=$totbruto+ $netto;
	  $totyard=$totyard+ $ukuran;
	  $totrol=$totrol+$row['roll'];
  ...
  {	
    ...
    <tr bgcolor="#9999FF">
      ...
      <td align="right"><?php echo number_format($row3['net_wight'],'2','.',','); ?></td>
    ...
    <?php
	if($row3['sisa']=="SISA" ||$row3['sisa']=="EXTRA"){$netto=0; $ukuran=0;}else{$netto=$row3['net_wight']; $ukuran=$row3['yard_'];}
	 $totbruto=$totbruto+ $netto;
	  $totyard=$totyard+ $ukuran;
	  $totrol=$totrol+$row['roll'];
