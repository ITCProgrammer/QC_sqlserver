...
<table  border="1" align="center" style="font-size:11px">
  ...
  $sql=mysql_query(" SELECT
	...
	AND (a.fromtoid = 'GUDANG KAIN JADI' OR a.fromtoid = 'PACKING' OR a.fromtoid='LAIN' OR a.fromtoid='INSPEK MEJA' OR a.fromtoid = 'GANTI STIKER') AND not ISNULL(b.transtatus) AND b.transtatus='1' ".$where8.$where7.$where6.$where5.$where4.$where3.$where.$where1." 
	GROUP BY
...
GROUP BY b.nokk");
    ...
    <input name="nokk" type="hidden" value="<?php echo $row['nokk'];?>"><a href="#"onClick="window.open('tempat-persediaan.php?nokk=<?php echo $row['nokk'];?>&ket=<?php echo $row['sisa'];?>','MyWindow','height=200,width=500');"><?php 
