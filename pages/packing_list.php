...
$(document).ready(function () {
    ...
    $('input[name="amount"], input[type="checkbox"]').on('change keyup blur', sumRows);
...
<table width="100%" border="0">
  ...
  <tr>
    ...
    <td><label for="select"></label>
      <select name="lprn" onchange="ganti()" >
     	 <option value="KAIN" selected="selected">Kain</option>
  ...
  <tr>
    ...
    <td>:</td>
    <td><input name="dono" type="text" id="dono" onchange="window.location='index1.php?p=packing_list&amp;dono='+this.value"  value="<?php echo $_GET['dono'];?>" size="20" tabindex="1"/>
      *</td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
...
while($rk=sqlsrv_fetch_array($sqlnokk)){?>
  ...
  <?php if($_GET['nokk4']!=""){?>
  <tr>
    <th height="26" align="right" valign="top" scope="row">No KK 5</th>
    <td>:</td>
...
INNER JOIN tbl_kite ON tbl_kite.id = tmp_detail_kite.id_kite
WHERE
	detail_pergerakan_stok.nokk = '".$_GET['nokk5']."'
AND typestatus = '2'
AND detail_pergerakan_stok.sisa != 'FKTH'
AND detail_pergerakan_stok.sisa != 'TH'");
$rowkk4=sqlsrv_fetch_array($snkk4);
...
while($rk=sqlsrv_fetch_array($sqlnokk)){?>
  ...
  ?>
  <tr>
    <th height="26" align="right" valign="top" scope="row">No KK 6</th>
    <td>:</td>
...
INNER JOIN tbl_kite ON tbl_kite.id = tmp_detail_kite.id_kite
WHERE
	detail_pergerakan_stok.nokk = '".$_GET['nokk6']."'
AND typestatus = '2'
AND detail_pergerakan_stok.sisa != 'FKTH'
AND detail_pergerakan_stok.sisa != 'TH'");
$rowkk4=sqlsrv_fetch_array($snkk4);
...
while($rk=sqlsrv_fetch_array($sqlnokk)){?>
  ...
  ?>
  <tr>
    <th height="26" align="right" valign="top" scope="row">No KK 7</th>
    <td>:</td>
...
INNER JOIN tbl_kite ON tbl_kite.id = tmp_detail_kite.id_kite
WHERE
	detail_pergerakan_stok.nokk = '".$_GET['nokk7']."'
AND typestatus = '2'
AND detail_pergerakan_stok.sisa != 'FKTH'
AND detail_pergerakan_stok.sisa != 'TH'");
$rowkk4=sqlsrv_fetch_array($snkk4);
...
while($rk=sqlsrv_fetch_array($sqlnokk)){?>
  ...
  ?>
  <tr>
    <th height="26" align="right" valign="top" scope="row">No KK 8 </th>
    <td>:</td>
...
INNER JOIN tbl_kite ON tbl_kite.id = tmp_detail_kite.id_kite
WHERE
	detail_pergerakan_stok.nokk = '".$_GET['nokk8']."'
AND typestatus = '2'
AND detail_pergerakan_stok.sisa != 'FKTH'
AND detail_pergerakan_stok.sisa != 'TH'");
$rowkk4=sqlsrv_fetch_array($snkk4);
...
while($rk=sqlsrv_fetch_array($sqlnokk)){?>
  ...
  ?>
  <tr>
    <th height="26" align="right" valign="top" scope="row">No KK 9</th>
    <td>:</td>
...
INNER JOIN tbl_kite ON tbl_kite.id = tmp_detail_kite.id_kite
WHERE
	detail_pergerakan_stok.nokk = '".$_GET['nokk9']."'
AND typestatus = '2'
AND detail_pergerakan_stok.sisa != 'FKTH'
AND detail_pergerakan_stok.sisa != 'TH'");
$rowkk4=sqlsrv_fetch_array($snkk4);
...
while($rk=sqlsrv_fetch_array($sqlnokk)){?>
  ...
  ?>
  <tr>
    <th height="26" align="right" valign="top" scope="row">No KK 10</th>
    <td>:</td>
...
INNER JOIN tbl_kite ON tbl_kite.id = tmp_detail_kite.id_kite
WHERE
	detail_pergerakan_stok.nokk = '".$_GET['nokk10']."'
AND typestatus = '2'
AND detail_pergerakan_stok.sisa != 'FKTH'
AND detail_pergerakan_stok.sisa != 'TH'");
$rowkk4=sqlsrv_fetch_array($snkk4);
...
<table width="100%" border="1">
    <tr>
      ...
      <th bgcolor="#9966CC" scope="col">Nokk</th>
      <th bgcolor="#9966CC" scope="col">SN</th>
      <th bgcolor="#9966CC" scope="col">No Warna/Warna</th>
      <th bgcolor="#9966CC" scope="col">LOT</th>
      <th bgcolor="#9966CC" scope="col">No Roll</th>
      <th bgcolor="#9966CC" scope="col">Netto (KG)</th>
      <th bgcolor="#9966CC" scope="col">Yard</th>
      <th bgcolor="#9966CC" scope="col">Satuan</th>
      <th bgcolor="#9966CC" scope="col">GRADE</th>
      <th bgcolor="#9966CC" scope="col">KET</th>
      <!--<th bgcolor="#9966CC" scope="col">POTONG</th>-->
      <th bgcolor="#9966CC" scope="col">
        <label for="checkbox"></label>
      <input type="checkbox" name="allbox" value="check" onClick="checkAll(0);" /><br /><font color="red">ALL</font></th>
    </tr>
   <?php
   if($_GET['dono']!='')
	{ $cwhere2.= $_GET['dono']; 
	...
	if($_GET['nokk5']!='')
	{ $cwherek5.= " OR (detail_pergerakan_stok.nokk ='".$_GET['nokk5']."' AND detail_pergerakan_stok.`status`='1') ";}else{ $cwherek5.= " "; }	
	if($_GET['nokk6']!='')
	{ $cwherek6.= " OR (detail_pergerakan_stok.nokk ='".$_GET['nokk6']."' AND detail_pergerakan_stok.`status`='1') ";}else{ $cwherek6.= " "; }	
	if($_GET['nokk7']!='')
	{ $cwherek7.= " OR (detail_pergerakan_stok.nokk ='".$_GET['nokk7']."' AND detail_pergerakan_stok.`status`='1') ";}else{ $cwherek7.= " "; }	
	if($_GET['nokk8']!='')
	{ $cwherek8.= " OR (detail_pergerakan_stok.nokk ='".$_GET['nokk8']."' AND detail_pergerakan_stok.`status`='1') ";}else{ $cwherek8.= " "; }
	if($_GET['nokk9']!='')
	{ $cwherek9.= " OR (detail_pergerakan_stok.nokk ='".$_GET['nokk9']."' AND detail_pergerakan_stok.`status`='1') ";}else{ $cwherek9.= " "; }	
	if($_GET['nokk10']!='')
	{ $cwherek10.= " OR (detail_pergerakan_stok.nokk ='".$_GET['nokk10']."' AND detail_pergerakan_stok.`status`='1') ";}else{ $cwherek10.= " "; }					
if($_GET['nopo']!='')
...
INNER JOIN tbl_kite ON tbl_kite.id = tmp_detail_kite.id_kite
WHERE
	(detail_pergerakan_stok.sisa !='FKTH' AND detail_pergerakan_stok.sisa !='TH' AND detail_pergerakan_stok.`status`='1' AND typestatus='2' AND ISNULL(detail_pergerakan_stok.`refno`) AND `tbl_kite`.`no_order`='".$cwhere2."'
 )".$cwhere.$cwherek2.$cwherek3.$cwherek4.$cwherek5.$cwherek6.$cwherek7.$cwherek8.$cwherek9.$cwherek10." ".$cwhere1."
...
IF
	...
	'In GKJ',
IF
	( a.`status` = 2 OR a.`status` = 3, 'Out GKJ', '' ) 
	) 
	) SEPARATOR '->' 
	) AS alur 
FROM
	( SELECT `status`,SN FROM detail_pergerakan_stok WHERE SN = '".$rowd['SN']."' ORDER BY `status` ASC ) a 
WHERE
	a.SN = '".$rowd['SN']."'	
	");
    ...
    <tr bgcolor="<?php echo $bgcolor;?>">
      ...
      <td align="center" ><?PHP 
		  if($rSN['alur']=="Packing->In GKJ"){
			 echo "<strong>".$rowd['sisa']."</strong><br><font color='blue'>Kain di Gudang KJ</font>";}
		  else if($rSN['alur']=="Packing->In GKJ->Out GKJ"){
			 echo "<strong>".$rowd['sisa']."</strong><br><font color='red'><i>Kain sudah Keluar</i></font>";}
		  else{
			 echo "<strong>".$rowd['sisa']."</strong><br><span class='blink'><font color='red'>Kain di Packing</font></span>";} ?></td>
      <!--<td align="center" ><input name="potong" type="button" value="..." onclick="NewWindow('pages/potong_kain.php?id=<?PHP echo $rowd['kd']; ?>&sn=<?PHP echo $rowd['SN']; ?>','MyWindow','width=150,height=220,left=300,top=300');"/></td>-->
      <td align="center" >
		  <input type="hidden" value="<?php echo number_format($rowd['weight'],'2','.',',');?>" size="6" name="amount"><input type="hidden" value="<?php echo number_format($rowd['yard_'],'2','.',',');?>" size="6" name="amount2">
		  <?php 
  ...
  <?php 
    ...
    <b>Total Qty yang di ceklist: <label id="result_label"><span id="total" style="color:red">0</span></label></b><br />
	<b>Total Panjang yang di ceklist: <label id="result_label"><span id="total2" style="color:red">0</span></label></b>	</font></p>
