...
<?php else : ?>
  ...
  sqlsrv_query($con, "INSERT INTO db_qc.log_laporan (ip_address, [date], range_date) VALUES('$ip_address','$date', '$range_date')");
  ?>
  ...
  <body>
    <div id="art-main">
      ...
      <div align="center">
        <font size="+3"><strong>LAPORAN HARIAN KAIN JADI <?php if ($_POST['ganti_stiker'] == "1") {
                                                            echo "GANTI STIKER";
                                                          } else if ($_POST['ganti_stiker'] == "2") {
                                                            echo "POTONG SISA";
                                                          } else if ($_POST['ganti_stiker'] == "3") {
                                                            echo "BS";
      ...
      <div align="right"><a href="data_kain_jadi_excel.php?awal=<?php echo $_POST['awal']; ?>&akhir=<?php echo $_POST['akhir']; ?>&no_order=<?php echo $_POST['order']; ?>&ganti_stiker=<?php echo $_POST['ganti_stiker']; ?>&shft=<?php echo $shft; ?>&ckKite=<?php echo $_POST['ckKite']; ?>">
          <font size="+2">Cetak Excel</font>
        </a>||
        <a href="data_kain_jadi_detail_now_excel.php?awal=<?php echo $_POST['awal']; ?>&akhir=<?php echo $_POST['akhir']; ?>&no_order=<?php echo $_POST['order']; ?>&ganti_stiker=<?php echo $_POST['ganti_stiker']; ?>&shft=<?php echo $shft; ?>&ckKite=<?php echo $_POST['ckKite']; ?>">
          <font size="+2">Cetak Detail NOW Excel</font>
        </a>
      </div>
      ...
      <table width="100%" class="display" id="datatables">
        ...
        <tbody>
          ...
          SUM(d.netto) as netto,
          STRING_AGG(lokasi, ', ') WITHIN GROUP (ORDER BY lokasi) as lokasi
        FROM
        ...
        ORDER BY
          ...
          while ($row = sqlsrv_fetch_array($sql)) {
            ...
            $mySql = sqlsrv_query($con, "SELECT mutasi_kain.tempat
            ...
            GROUP BY mutasi_kain.tempat");
            $myBlk = sqlsrv_fetch_array($mySql);

            $sqlPtg = sqlsrv_query($con, "SELECT
                SUM(CASE WHEN grade = 'C' THEN weight ELSE 0 END) AS grd_c,
                SUM(CASE WHEN grade IN ('A', 'B', '') THEN weight ELSE 0 END) AS grd_ab,
                SUM(CASE WHEN grade IN ('A', '') THEN weight ELSE 0 END) AS grd_a,
            ...
            $sqlPtgs = sqlsrv_query($con, "SELECT
                ...
                SUM(CASE WHEN grade IN ('A', '') THEN weight ELSE 0 END) AS grd_a,
                SUM(CASE WHEN grade = 'B' THEN weight ELSE 0 END) AS grd_b,
                SUM(ISNULL(yard_, 0)) AS yard_
            FROM db_qc.detail_pergerakan_stok 
            WHERE nokk = '" . $row['nokk'] . "' 
              AND transtatus = '3' 
              AND id_ref = '" . $row['id'] . "' 
              -- Gunakan CAST jika sisa adalah tipe data numeric/bigint
