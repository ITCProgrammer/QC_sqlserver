...
$db_name="TM";
 set_time_limit(600);
	...
	if(substr($_GET['bagian'],0,7)=="PACKING"){$bgn="not";$bagian1="1";}else{$bgn=" ";$bagian1="2";} 
  $sql1=mssql_query("select productprop.batchno,productprop.pono, productprop.rollno,stockmovementdetails.weight,stockmovementdetails.refno from stockmovement 
...
where stockmovement.wid=22 and stockmovement.transactiontype=4 and stockmovement.transactionstatus=1 and CAST(TM.dbo.stockmovement.[note] AS VARCHAR(8000)) $bgn like '%IM%'
and productprop.batchno='$_GET[id]'");
  $row1=mssql_fetch_array($sql1);
  $jumlah=mssql_num_rows($sql1);
   ?>
    <script>
   
          function ganti(id,thn1,bln1,hr1,thn2,bln2,hr2,sift,bagian)
{     var yrd= document.forms['form2']['yrd_'].value;  
var x,meter;
if(yrd=="meter"){
	window.location.href="form-Packing?p=mutasi_detail_meter_online&id="+ id +"&tgl1="+ thn1 +"-"+ bln1 +"-"+ hr1 +"&tgl2="+ thn2 +"-"+ bln2 +"-"+ hr2 +"&sift="+ sift+"&bagian="+ bagian;
	}

}
           </script>  
      <div align="center"><b>BUKTI MUTASI KAIN DETAIL ONLINE <?php echo $_GET['bagian'];?></b> </div><div><b>Tanggal : <?php echo $_GET['tgl1']." s/d ".$_GET['tgl2']; ?> <br>
      SHIFT : <?php echo $_GET['sift'];?><br>
      No Kartu Kerja : <?php echo $row1['batchno'];?><br>
      No Order :<?php echo $row1['pono'];?><br>
...
left join productprop on productprop.id = stockmovementdetails.productpropid
where stockmovement.wid=22 and stockmovement.transactiontype=4 and stockmovement.transactionstatus=1 and CAST(TM.dbo.stockmovement.[note] AS VARCHAR(8000)) $bgn like '%IM%'
and productprop.batchno='$_GET[id]' Order by productprop.rollno asc");
  ...
  ?><div>
      <form name="form2">
        Satuan : 
        <select name="yrd_" 
    onchange="ganti(<?php echo $_GET['id']; ?>,<?php echo $thn1; ?>,<?php echo $bln1; ?>,<?php echo $hr1; ?>,<?php echo $thn2; ?>,<?php echo $bln2; ?>,<?php echo $hr2; ?>,<?php echo $_GET['sift']; ?>,<?php echo $bagian1; ?>)">
