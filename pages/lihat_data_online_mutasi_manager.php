...
<body>
    <div id="art-main">
      <input type="button" class="art-button" onClick="window.location.href='../index_login.php?act=logout'" value="Logout"/>
                            <?php if($_SESSION['status']==0){ ?><input type="button" class="art-button" onClick="window.location.href='../index1.php'" value="Home"/><input type="button" class="art-button" onClick="window.location.href='../index1.php?p=laporan_harian_online'" value="Laporan"/><br /><?php } ?>
  ...
  <?php $tgl_cetak1= $_POST['awal'];
   if($_POST['sift']=="3"){$tgl_cetak2=date("Y-m-d",strtotime ( '1 day' , strtotime ($tgl_cetak1)));}else{$tgl_cetak2= $_POST['awal'];} ?>
    <td colspan="22"><b>Tanggal : <?php echo  date("d-M-Y", strtotime($tgl_cetak1))." s/d ".date("d-M-Y", strtotime($tgl_cetak2)); ?><br> SHIFT : <?php echo $_POST['sift'];?></b></td>
  </tr>
...
where wid='12' and transactiontype='7' and processcontrolbatches.documentno='$row[batchno]'");
	...
	echo number_format($yd,'2','.',',');
	?></td>
    <td>
	 <a href="../index1.php?p=mutasi_detail_online&id=<?php echo $row['batchno'];?>&tgl1=<?php echo $tgl_cetak1; ?>&tgl2=<?php echo $tgl_cetak2; ?>&sift=<?php echo $_POST['sift'];?>&bagian=<?php echo $_POST['bagian']; ?>"><?php echo $row['batchno']; ?></a>
 ...
 <!--   <a href="pages/cetak/cetak_online_mutasi.php?tgl1=<?php /* echo $tgl_cetak1; ?>&tgl2=<?php echo $tgl_cetak2; ?>&user_name=<?php echo $_POST['user_name']; ?>&sift=<?php echo $_POST['sift'];  */?>" target="_blank">Cetak</a> -->                       
