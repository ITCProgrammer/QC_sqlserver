...
<!-- <input type="submit" class="art-button" onClick="window.history.go(-1); return false;" value="Pengiriman" /> -->
<input type="button" class="art-button" onClick="window.location.href='../index1.php?p=pengirimMKTNew'" value="Pengiriman" />
<div align="center">LAPORAN HARIAN PENGIRIMAN <br />
BULAN <?php echo $bulan[$bln]." ".$thn; ?> </div> 
<br />
<a href="lihat_data_pengiriman_mkt_excel.php?awal=<?php echo $_GET['awal'];?>&akhir=<?php echo $_GET['akhir'];?>&no_sj=<?php echo $_GET['no_sj'];?>&buyer=<?php echo $_GET['buyer'];?>">CETAK EXCEL</a>
<div align="right">
    <input type="submit" value="Pilih Cetak" name="save" id="save"/>
    <a href="cetak_surat_jalan_mkt_dummy.php?no_sj1=<?php echo $_POST['cek'][0]; ?>&tgl_kirim1=<?php if($_POST['cek'][0]!=""){echo $_POST['kirim'][0];}?>&no_sj2=<?php echo $_POST['cek'][1]; ?>&tgl_kirim2=<?php if($_POST['cek'][1]!=""){echo $_POST['kirim'][1];}?>&no_sj3=<?php echo $_POST['cek'][2]; ?>&tgl_kirim3=<?php if($_POST['cek'][2]!=""){echo $_POST['kirim'][2];}?>&no_sj4=<?php echo $_POST['cek'][3]; ?>&tgl_kirim4=<?php if($_POST['cek'][3]!=""){echo $_POST['kirim'][3];}?>&no_sj5=<?php echo $_POST['cek'][4]; ?>&tgl_kirim5=<?php if($_POST['cek'][4]!=""){echo $_POST['kirim'][4];}?>" target="_blank">Cetak Surat Jalan</a>
</div>
<table width="100%" border="0" class="display" id="datatables">
 <thead>
  ...
  $ttglm=date("d", $newdate);
    if($_GET['awal']!=""){
        $tgll= " a.tgl_update='".$_GET['awal']."' ";
        }else{$tgll= " a.tgl_update='' ";}	  
    if($_GET['no_sj']!=""){
        $sj= " AND a.no_sj='".$_GET['no_sj']."' ";
        }	 
...
$row2=mysqli_fetch_array($sql2);	
  ...
  if($_GET['buyer']!=""){
    $buyer2= " AND buyer LIKE '%".$_GET['buyer']."%' ";
  }	 
...
while($row3=mysqli_fetch_array($sqlbr)){
    ...
    if($_GET['awal']!="" AND $row3['satuan_mkt']==''){
        $sqlupdate=mysqli_query($con,"UPDATE tbl_pengiriman SET 
            ...
            WHERE tgl_buat BETWEEN '".$_GET['awal']."' AND '".$_GET['akhir']."' AND no_order='".$rowmkt['DocumentNo']."' AND warna='".$rowmkt['Color']."' AND no_po='".$rowmkt['PONumber']."' AND no_item='".$rowmkt['ProductCode']."'
        ");
    }
    if($_GET['no_sj']!="" AND $row3['satuan_mkt']==''){
        $sqlupdate=mysqli_query($con,"UPDATE tbl_pengiriman SET 
            `satuan_mkt`='".$rowmkt['UnitName']."',
            `currency`='".$rowmkt['Symbol']."',
            `price`='".$rowmkt['UnitPrice']."'
            WHERE no_sj='".$_GET['no_sj']."' AND no_order='".$rowmkt['DocumentNo']."' AND warna='".$rowmkt['Color']."' AND no_po='".$rowmkt['PONumber']."' AND no_item='".$rowmkt['ProductCode']."'
        ");
    }
  ...
  <tr bgcolor="<?php echo $bgcolor;?>" >
    ...
    <?php echo $row3['no_sj']; ?>
    <br><?php if(strpos($row3['jenis_kain'],'FLAT KNIT') === false){?><input type="checkbox" name="cek[]" value="<?php echo $row3['no_sj']; ?>" 
        <?php if (isset($_POST['cek']) && in_array($row3['no_sj'], $_POST['cek'])): ?>
        checked="checked"
        <?php endif; ?>/>
        <input type="hidden" name="kirim[]" value="<?php echo $row3['tgl_kirim']; ?>"/>
        <?php } ?>
    </td>
    ...
    <td <?php if($row3['approve_acc']=="Approve"){?> bgcolor="#FF962C" <?php }?>>
      <?php if($row3['approve_acc']=="Approve" AND $_SESSION['username']=="mktspv"){?><a href="#" onClick="window.open('edit_price.php?id=<?php echo $row3['id']; ?>&nosj=<?php echo $row3['no_sj']; ?>','MyWindow','height=200,width=500,top=250,left=500');">
      UBAH <?php }else if($row3['approve_acc']=="" OR $row3['approve_acc']==""){?><a href="#" onClick="window.open('edit_price.php?id=<?php echo $row3['id']; ?>&nosj=<?php echo $row3['no_sj']; ?>','MyWindow','height=200,width=500,top=250,left=500');">
      UBAH <?php }else{echo "&nbsp;";} ?>
      </a>
    </td>
  </tr>
