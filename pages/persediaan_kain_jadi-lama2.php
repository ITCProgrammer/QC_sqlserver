    ...
    <div id="art-main">
      <div class=""></div>
            ...
            <input type="button" class="art-button" onClick="window.location.href='../index_login.php?act=logout'" value="Logout"/>
                   ...
                   <center>
                   <b>LAPORAN PERSEDIAAN KAIN JADI <?php  if($_POST[bs]==1){echo"BS";}?></b>
                   </center>    
   <div align="right"><a href="persediaan_kain_jadi_excel.php?awal=<?php echo $_POST['awal'];?>&akhir=<?php echo $_POST['akhir'];?>&no_order=<?php echo $_POST['no_order'];?>&blok=<?php echo $_POST['blok'];?>&ket=<?php echo $_POST['ket'];?>&no_item=<?php echo $_POST['no_item'];?>&no_warna=<?php echo $_POST['no_warna'];?>&lbr=<?php echo $_POST['lbr'];?>&grms=<?php echo $_POST['grms'];?>&buyer=<?php echo $_POST['buyer'];?>&bs=<?php echo $_POST['bs'];?>">Cetak Excel</a>&nbsp;||&nbsp;<a href="persediaan_kain_jadi_detail_excel.php?awal=<?php echo $_POST['awal'];?>&akhir=<?php echo $_POST['akhir'];?>&no_order=<?php echo $_POST['no_order'];?>&blok=<?php echo $_POST['blok'];?>&ket=<?php echo $_POST['ket'];?>&no_item=<?php echo $_POST['no_item'];?>&no_warna=<?php echo $_POST['no_warna'];?>&lbr=<?php echo $_POST['lbr'];?>&grms=<?php echo $_POST['grms'];?>&buyer=<?php echo $_POST['buyer'];?>&bs=<?php echo $_POST['bs'];?>">Cetak Detail</a></div> 
...
mysql_select_db("db_qc")or die("Gagal Koneksi");
  ...
  <tr align="center" bgcolor="#0099FF">
    <td rowspan="2" bgcolor="#3366CC">NO</td>
  ...
  $sql=mysql_query(" SELECT
	...
	c.warna,c.lebar,c.berat,c.no_item,b.id_stok,a.catat,a.id
	FROM
	pergerakan_stok a
  ...
  while($row=mysql_fetch_array($sql))
  {
	   ...
	   $mySql =mysql_query("SELECT tempat,catatan FROM mutasi_kain WHERE nokk='$row[nokk]' AND keterangan='$row[sisa]' AND not tempat='' order by id desc");
	   $myBlk = mysql_fetch_array($mySql);
	  	$mySqlC =mysql_query("SELECT tempat,catatan FROM mutasi_kain WHERE nokk='$row[nokk]' AND keterangan='$row[sisa]' order by id desc");
	...
	AND (a.fromtoid = 'GUDANG KAIN JADI' OR a.fromtoid = 'PACKING' OR a.fromtoid='LAIN' OR a.fromtoid='INSPEK MEJA' OR a.fromtoid ='GANTI STIKER' OR a.fromtoid ='POTONG SISA')
	GROUP BY
	b.sisa,b.id_stok
	ORDER BY
	a.id ");
...
GROUP BY b.nokk");
    ...
    <td><b title="<?php if($myBlk1['no_po']!=""){echo $myBlk1['no_po'];}else{echo $myBlk2['no_po'];}?>"><a href="#" onClick="window.open('tambah-catat.php?id=<?php echo $row['id_stok'];?>&nokk=<?php echo $row['nokk'];?>&ket=<?php echo $row['sisa'];?>&no_po=<?php if($myBlk1['no_po']!=""){echo $myBlk1['no_po'];}else{echo $myBlk2['no_po'];}?>','MyWindow','height=300,width=550,top=250,left=500');"><?php if($myBlk1['no_po']!=""){echo substr($myBlk1['no_po'],0,7)."...";}else{echo substr($myBlk2['no_po'],0,7)."...";}?></a></b></td>
 ...
 <tr bgcolor="#99FFFF">
  ...
  <tr bgcolor="#99FFFF">
    <td colspan="23" bgcolor="#CCFF99"><b>( Roll : <?php echo  number_format($totrol);  ?> ) (GRADE A+B: <?php echo  number_format($totab,'2','.',',');  ?> Kg, Roll: <?php echo  number_format($rolab);  ?>)  (GRADE C: <?php echo  number_format($tota,'2','.',',');  ?> Kg, Roll: <?php echo  number_format($rolac);  ?>) (TOTAL : <?php echo  number_format($totab+$tota,'2','.',',');  ?> Kg) </b></td>
    </tr>
