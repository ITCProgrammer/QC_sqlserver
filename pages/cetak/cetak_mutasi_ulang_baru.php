...
@media print {
  ...
  <body>
    ...
    $lth=mysql_query("select pergerakan_stok.tgl_update as tanggal_update ,pergerakan_stok.userid as user_packing ,mutasi_kain.no_mutasi,now() as tgl_skrng
from mutasi_kain
...
GROUP BY no_mutasi");
          ...
          <table width="100%" class="table-list1">
            <tr>
              <td width="79%"><b>Tanggal :
                  <?php echo date("d F Y", strtotime($rowlth['tanggal_update']));?> <br>GROUP SHIFT:
                  <?php
 echo $rowlth['user_packing']; ?> <br> SHIFT :
                  <?php
  /* if(date("H:i:s",strtotime($rowlth['tanggal_update']))>="23:00:00" && date("H:i:s",strtotime($rowlth['tanggal_update']))<="06:59:59")
  {$rsift=3;}
  else  */if (date("H:i:s", strtotime($rowlth['tanggal_update']))>="07:00:00" && date("H:i:s", strtotime($rowlth['tanggal_update']))<="14:59:59") {
      $rsift=1;
  } elseif (date("H:i:s", strtotime($rowlth['tanggal_update']))>="15:00:00" && date("H:i:s", strtotime($rowlth['tanggal_update']))<="22:59:59") {
      $rsift=2;
  } else {
      $rsift=3;
  }
 echo $rsift;?>
      ...
      <tr align="center" bgcolor="#CCCCCC">
        ...
        <td rowspan="3" bgcolor="#F5F5F5">Lot</td>
        ...
        <td rowspan="3" bgcolor="#F5F5F5">
          <?php if ($rowlth['user_packing']=="KRAH") {
     echo "PCS";
 } else {
     echo "Yard / Meter";
 } ?>
        </td>
        <td rowspan="3" bgcolor="#F5F5F5">No.Kartu Kerja</td>
 ...
 /* if($usr=="INS"){$kt="AND detail_pergerakan_stok.ket='INSPEK'"; $ktc="INSPEK";}else{$kt=""; $ktc="";}  */
 /*user*/
  if ($usr=="INS" or $usr=="ins") {
      $kt="AND detail_pergerakan_stok.ket='INSPEK'";
      $ktc="INSPEK";
      $usr1="INS";
  } elseif ($usr=="KRA" or $usr=="kra") {
      $kt="";
      $ktc="";
      $usr1="KRA";
  } else {
...
$grc=0;
  while ($row=mysql_fetch_array($sql)) {
      $sqlket=mysql_query("select nokk,ket_c,sisa from detail_pergerakan_stok where nokk ='$row[nokk]' and ket_c !='' and sisa !='TH' and sisa !='FKTH' and grade='C' and ket ='$ktc'
GROUP BY ket_c");
      $rowket=mysql_fetch_array($sqlket); ?>
      <tr>
        <td>
          <font size="-2">
            <?php echo $row['no_mc']; ?>
        ...
        <td>
          <font size="-2">
            <?php echo substr($row['no_po'], 0, 13)." ".substr($row['no_po'], 13, 13)." ".substr($row['no_po'], 26, 13); ?>
          </font>
        </td>
        <td>
          <font size="-2">
            <?php echo substr($row['no_order'], 0, 6)." ".substr($row['no_order'], 6, 15); ?>
          </font>
        </td>
        <td>
          <font size="-2">
        ...
        <td>
          <font size="-2">
            <?php echo substr($row['no_warna'], 0, 7)." ".substr($row['no_warna'], 7, 20); ?>
        </td>
        <td>
          <font size="-2">
            <?php echo substr($row['warna'], 0, 7)." ".substr($row['warna'], 7, 20); ?>
        </td>
        <td>
          <font size="-2">
            <?php if ($rowlth['user_packing']=="KRAH") {
        ...
        <td align="right">
          <font size="-2">
            <?php $rol=$row['tot_rol'];
      if (($row['jml_grd_c']>0) and ($row['jml_ab']>0)) {
          $rol1=$row['jml_ab']."+".$row['jml_grd_c'];
      } elseif ($row['jml_grd_c']>0) {
          $rol1=$row['jml_grd_c'];
      } else {
          $rol1=$row['jml_ab'];
      }
      echo $rol1; ?>
        ...
        <td align="right">
          <font size="-2">
            <?php echo number_format($row['bruto'], '2', '.', ','); ?>
          </font>
        </td>
        <td align="right">
          <font size="-2">
            <?php	echo number_format($row['grd_a'], '2', '.', ','); ?>
          </font>
        </td>
        <td align="right">
		  <font size="-2">
            <?php	echo number_format($row['grd_b'], '2', '.', ','); ?>
          </font></td>
        <td align="right">
          <font size="-2">
            <?php echo number_format($row['grd_c'], '2', '.', ','); ?>
          </font>
        </td>
        <td>
          <font size="-2">
     ...
     if ($row['sisa']=="SISA" || $row['sisa']=="FKSI" || $row['sisa']=="FOC") {
	  ...
	  $totqtya=$totqtya+$row['grd_a'];
	  $totqtyb=$totqtyb+$row['grd_b'];
      $totqtyc=$totqtyc+$row['grd_c'];
      if ($row['satuan']=='Meter') {
  ...
  ?>
      ...
      <tr bgcolor="#99FFFF">
        ...
        <td align="right" bgcolor="#FFFFFF"><b>
            <?php echo number_format($totbruto, '2', '.', ',');?></b></td>
        <td align="right" bgcolor="#FFFFFF"><b>
    ...
    <table width="100%" border="0" class="table-list1">
      ...
      <tr>
        ...
        <td colspan="3" align="center">
          <?php echo date("d-M-Y", strtotime($rowlth['tanggal_update']));?>
        </td>
