...
if(lprn=="BONGKARAN"){
	window.location.href="index1.php?p=kain_bs_keluar_bongkaran_barcode";
	}
}
           </script> 
</head>
...
if(isset($_POST['btnTambah'])){
		...
		$cekSql ="SELECT * FROM tmp_detail_pergerakan_stok WHERE SN='$Barcode' AND transtatus='12' AND userid='".$usernm."'"; 
		...
		//if ((mysql_num_rows($cekQry) >= 1)  or (mysql_num_rows($cekQry0)<=1) or (mysql_num_rows($cekQry2)>=1)) {
			if ((mysql_num_rows($cekQry) >= 1) or (mysql_num_rows($cekQry2)>=1)) {
		...
		//	}
			...
			$mySql ="SELECT * FROM detail_pergerakan_stok WHERE SN='$Barcode' AND transtatus='11'";
			...
			if ($myQty >= 1) {
				...
				(id_detail_kj,weight,yard_,no_roll,grade,satuan,sisa,SN,ket,nokk,ket_c,transtatus,userid) 
							...
							, '$myRow[grade]', '$myRow[satuan]', '$myRow[sisa]', '$myRow[SN]', '$myRow[ket]', '$myRow[nokk]', '$myRow[ket_c]','12','".$usernm."')";
...
if(isset($_POST['btnSimpan'])){
	...
	$tmpSql ="SELECT COUNT(*) As qty FROM tmp_detail_pergerakan_stok WHERE userid='".$usernm."' AND transtatus='12'";
	...
	if (count($pesanError)>=1 ){
		...
		$tmpSql1 ="SELECT id  FROM pergerakan_stok WHERE typestatus='13' and userid='".$usernm."' ORDER BY id DESC" ;
		...
		while ($tmpData = mysql_fetch_array($tmpQry)) {
			...
			$itemSqlU="UPDATE detail_pergerakan_stok 
								SET transtatus='10' 
								where  (transtatus !='12' or transtatus !=NULL)  
								and SN='$dataSN'";
...
<fieldset>
  ...
  <tr>
    ...
    <td><a href="index1.php?p=data_kain_bs_keluar_barcode">Ubah Kain Keluar</a></td>
  </tr>
  <tr>
    <td colspan="4"></td>
  </tr>
...
INNER JOIN tbl_kite b ON c.id_kite = b.id WHERE a.transtatus='12' and a.userid='".$usernm."' order by a.no_roll ASC");
	$no=1;
	 ...
	 ?>
      <tr bgcolor="<?php echo $bgcolor; ?>">
        ...
        <td align="center" >&nbsp;&nbsp;<a  onclick="return confirm('ANDA YAKIN AKAN MENGHAPUS DATA INI ... ?')" href="pages/hapus_tmp_b.php?nokk=<?PHP echo $rowd['nokk']; ?>&amp;idtmp=<?PHP echo $rowd['id']; ?>&amp;bs=1">HAPUS</a>&nbsp;&nbsp;</td>
      </tr>
      <?php 
