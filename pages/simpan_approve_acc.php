...
if($_POST['submit']=='SIMPAN'){
    ...
    ORDER BY no_sj asc");
    $n=0; 
    while($row=sqlsrv_fetch_array($sqlbr)){
    if($_POST['check'][$n]!=''){
        $pos=strpos($_POST['check'][$n], ",");
        $no_sj=substr($_POST['check'][$n],0,$pos);
        $tgl_kirim=substr($_POST['check'][$n],$pos+1,50);
        $dataupdate=sqlsrv_query($con,"UPDATE tbl_pengiriman SET approve_acc='Approve', ipaddress_acc='".$_SERVER['REMOTE_ADDR']."', tgl_approve_acc=now() where no_sj='$no_sj' AND tgl_kirim='$tgl_kirim'")or die("Gagal!  $id");
          $n++;}else{$n++;}
    }
    if($dataupdate)
    {
