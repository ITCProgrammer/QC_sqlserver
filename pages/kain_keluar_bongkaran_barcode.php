	...
	<script>
		function ganti() {
			...
			if (lprn == "PENGIRIMAN_LOKAL") {
				window.location.href = "index1.php?p=kain_keluar_barcode_lokal";
...
<body>
	...
	function sn()
	{
		...
		// $con=sqlsrv_connect("10.0.0.10","dit","4dm1n","db_qc");	
		include(__DIR__ . '../../koneksi.php');
		$qtgl = sqlsrv_query($con, "SELECT FORMAT(GETDATE(), 'yy') AS tgl");
		$dttgll = sqlsrv_fetch_array($qtgl, SQLSRV_FETCH_ASSOC);
		$format = $dttgll['tgl'] . "2";
		$sql = sqlsrv_query($con, "SELECT TOP 1 SN FROM db_qc.detail_pergerakan_stok 
	WHERE SUBSTRING(CAST(SN AS VARCHAR(20)), 1, 3) LIKE '%" . $format . "%' ORDER BY SN DESC") or die(sqlsrv_errors());
		$d = sqlsrv_num_rows($sql);
		if ($d > 0) {
			$r = sqlsrv_fetch_array($sql);
			$d = $r['SN'];
			$str = SUBSTRING($d, 3, 10);
			$Urut = (int)$str;
		} else {
			$Urut = 0;
		}
	...
	if (isset($_POST['btnTambah'])) { #AWAL

		# Jika sudah pernah dipilih		
		$cekSql = "SELECT * FROM db_qc.tmp_detail_pergerakan_stok WHERE SN='$_POST[barcode]' AND transtatus='2' 
		AND userid='" . $usernm . "'";
		$cekQry = sqlsrv_query($con, $cekSql) or die("Gagal Query" . sqlsrv_errors());
		$cekSql1 = "SELECT * FROM db_qc.detail_pergerakan_stok WHERE SN='$_POST[barcode]' AND (transtatus='1' or transtatus='0')";
		$cekQry1 = sqlsrv_query($con, $cekSql1) or die("Gagal Query d" . sqlsrv_errors());
		$cekSql0 = "SELECT * FROM db_qc.detail_pergerakan_stok WHERE SN='$_POST[barcode]' ";
		$cekQry0 = sqlsrv_query($con, $cekSql0) or die("Gagal Query d" . sqlsrv_errors());
		$c0 = sqlsrv_fetch_array($cekQry0);
		$cekSql2 = "SELECT * FROM db_qc.detail_pergerakan_stok WHERE SN='$_POST[barcode]' AND transtatus='2'";
		...
		if ((sqlsrv_num_rows($cekQry) >= 1)  or (sqlsrv_num_rows($cekQry0) <= 1) or (sqlsrv_num_rows($cekQry2) >= 1)) {
			...
			$mySql = "SELECT * FROM db_qc.detail_pergerakan_stok WHERE SN='$_POST[barcode]' AND transtatus='1'";
	...
	if (isset($_POST['btnSimpan'])) { #AWAL1
		$pesanError = array();
		...
		if ($_POST['jns_bongkar'] == "POTONG KAIN" or $_POST['jns_bongkar'] == "POTONG SAMPLE") {
			$qrc = sqlsrv_query($con, "SELECT weight FROM db_qc.tmp_detail_pergerakan_stok WHERE userid='" . $usernm . "'");
			$tmpDataC = sqlsrv_fetch_array($qrc);
		...
		$tmpSql = "SELECT COUNT(*) As qty FROM db_qc.tmp_detail_pergerakan_stok WHERE userid='" . $usernm . "'";
		...
		if (count($pesanError) >= 1) {
			...
			if ($_POST['jns_bongkar'] != "POTONG KAIN") {
				...
				$tmpSql1 = "SELECT id  FROM db_qc.pergerakan_stok WHERE typestatus='3' 
			...
			$tmpQry = sqlsrv_query($con, $tmpSql) or die("Gagal Query Tmp" . sqlsrv_errors());
			while ($tmpData = sqlsrv_fetch_array($tmpQry)) { #AWAL WHILE
				if ($_POST['qty_p'] == "") { #AWAL
				...
				if ($_POST['qty_p'] != "") {
					$grm = sqlsrv_query($con, "SELECT lebar,berat FROM db_qc.tbl_kite where nokk='$tmpData[nokk]'") or die("gagal cari data");
					$rgrm = sqlsrv_fetch_array($grm);
					$x = (($rgrm['lebar'] + 2) * $rgrm['berat']) / 43.05;
					$x1 = (1000 / $x);
					$yard = $x1 * $_POST['qty_p'];
					$meter = $yard * (768 / 840);
					if ($tmpData['satuan'] == "Yard") {
						$yard_ = round($yard, '2');
					} else {
						$yard_ = round($meter, '2');
					}
					...
					$dataKetC	= str_replace("'", "''", $tmpData['ket_c']);
					if ($_POST['jns_bongkar'] == "POTONG KAIN") {
						$qrid = sqlsrv_query($con, "SELECT id_stok as id FROM db_qc.detail_pergerakan_stok where SN='$tmpData[SN]' AND transtatus='1'");
						$tmpData1 = sqlsrv_fetch_array($qrid);
						$st = 1;
						$pk = "PK";
					} else {
						$st = 3;
					...
					$qrid1 = sqlsrv_query($con, "SELECT id_stok as id FROM db_qc.detail_pergerakan_stok where SN='$tmpData[SN]' AND transtatus='1'");
					$tmpData11 = sqlsrv_fetch_array($qrid1);
					$dataID		= $tmpData1['id'];
					...
					$itemSqlUPS = "UPDATE db_qc.detail_pergerakan_stok 
								SET  weight='$dataBerat', 
									 yard_='$dataYard'
								where  (transtatus ='1')  
								and SN='$dataSN'";
					sqlsrv_query($con, $itemSqlUPS) or die("Gagal Query UPdate Potong Sample" . sqlsrv_errors());
				} else {
			...
			<table width="903" border="0" cellpadding="0">
					...
					<td width="55%" rowspan="7">
						<div class="rol">
							<table width="417" border="0" cellpadding="2" cellspacing="3">
								...
								<?php
								//tambahan
								$datav = sqlsrv_query($con, "SELECT * FROM db_qc.detail_pergerakan_stok WHERE transtatus='1' and nokk='" . $_GET['nokk'] . "' order by no_roll ASC");
								$n = 1;
								while ($rowdv = sqlsrv_fetch_array($datav)) { ?>
									<?php
				...
				<tr>
					...
					<td><label for="jns_bongkar"></label>
						<select name="jns_bongkar" id="jns_bongkar" onchange="window.location='index1.php?p=kain_keluar_bongkaran_barcode&amp;jns='+this.value">
							...
							<option value="GANTI STIKER" <?php if ($_GET['jns'] == "GANTI STIKER") {
																echo "SELECTED";
															} ?>>GANTI STIKER</option>
							<option value="REVISI STIKER" <?php if ($_GET['jns'] == "REVISI STIKER") {
																echo "SELECTED";
															} ?>>REVISI STIKER</option>
							<option value="POTONG SAMPLE" <?php if ($_GET['jns'] == "POTONG SAMPLE") {
																echo "SELECTED";
															} ?>>POTONG SAMPLE</option>
							<option value="TOLAKAN" <?php if ($_GET['jns'] == "TOLAKAN") {
														echo "SELECTED";
													} ?>>TOLAKAN</option>
							<option value="BS" <?php if ($_GET['jns'] == "BS") {
													echo "SELECTED";
												} ?>>BS</option>
						</select>
					</td>
				...
				<tr>
					...
					<td><select name="nokk" onchange="window.location='index1.php?p=kain_keluar_bongkaran_barcode&amp;jns=<?php echo $_GET['jns']; ?>&amp;dono=<?php echo $_GET['dono']; ?>&amp;nokk='+this.value">
							...
							<?php
							$sqlnokk = sqlsrv_query($con, "SELECT a.nokk FROM db_qc.detail_pergerakan_stok a
LEFT JOIN db_qc.tmp_detail_kite b ON b.id=a.id_detail_kj
LEFT JOIN db_qc.tbl_kite c ON c.id=b.id_kite
WHERE a.transtatus='1' AND c.no_order='$_GET[dono]'
GROUP BY a.nokk ORDER BY a.nokk ASC");
							while ($rk = sqlsrv_fetch_array($sqlnokk)) { ?>
								<option value="<?php echo $rk['nokk']; ?>" <?php if ($_GET['nokk'] == $rk['nokk']) {
																				echo "SELECTED";
																			} ?>><?php echo $rk['nokk']; ?></option>
							<?php  } ?>
						</select></td>
				</tr>
				<tr>
		...
		<div class="rol">
			<table width="" border="0" cellpadding="1" cellspacing="1">
				...
				<?php
				//tambahan
				$data = sqlsrv_query($con, "SELECT * FROM db_qc.tmp_detail_pergerakan_stok a
	 WHERE a.transtatus='2' and a.userid='" . $usernm . "' order by a.no_roll ASC");
				$no = 1;
				while ($rowd = sqlsrv_fetch_array($data)) {
					$dtl = sqlsrv_query($con, "SELECT b.no_order,b.no_po,b.warna FROM db_qc.tmp_detail_kite c 
INNER JOIN db_qc.tbl_kite b ON c.id_kite = b.id WHERE c.id='$rowd[id_detail_kj]' ORDER BY c.no_roll ASC");
					$rowdtl = sqlsrv_fetch_array($dtl);
				?>
					<?php
					$bgcolor = ($c++ & 1) ? '#33CCFF' : '#FFCC99';
					?>
					<tr bgcolor="<?php echo $bgcolor; ?>">
						...
						<td align="center"><?PHP if ($_GET['jns'] == "POTONG SAMPLE" or $_GET['jns'] == "POTONG KAIN") {
												echo "<input type='text' value='$rowd[weight]' name='qty_p' size='10'>";
											} else {
												echo number_format($rowd['weight'], '2', '.', ',');
											} ?></td>
