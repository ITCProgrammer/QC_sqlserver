...
<tr style="height:0.1in;"><td valign="top"><center>
  <table width="100%" border="0" cellpadding="0"  class="noborder">
    ...
    <tr>
      ...
      <td style="text-align:left;font-size:7px;">: 01</td>
    </tr>
    <tr>
      <td>&nbsp;</td>
      <td align="center">&nbsp;</td>
      <td style="text-align:left;font-size:7px;">Tgl Terbit</td>
      <td style="text-align:left;font-size:7px;">: 01-08-16</td>
    </tr>
  </table>
  <table width="100%" border="0" cellpadding="0"  class="noborder">
  <thead>
...
E-MAIL: marketing@indotaichen.com Web-site: www.indotaichen.com</font></td>
  ...
  <u>PACKING LIST</u><br />
  <u>NO. :<?php echo $rslq['listno']?> </u></center>
    <table width="100%" border="0" cellpadding="0"  class="noborder">
      <tr>
        <td width="58%">BUYER: <?php echo $rslq['pelanggan'];?></td>
        <td width="42%" style="text-align:right">Tangerang, <?php echo Indonesia2Tgl($dtgl['tgl']);?></td>
      </tr>
      </table>
    </td></tr>
<tr><td valign="top" >
...
WHERE Row = 2");
$rjns=sqlsrv_fetch_array($conn,$sqljns);
	
	$sqla=sqlsrv_query($con,"SELECT count(no_lot)as roll,sum(detail_pergerakan_stok.weight) as berat,sum(detail_pergerakan_stok.yard_) as panjang,detail_pergerakan_stok.satuan FROM packing_list 
...
GROUP BY detail_pergerakan_stok.satuan ");
 ...
 $sqla1=sqlsrv_query($con,"SELECT count(no_lot)as roll,sum(detail_pergerakan_stok.weight) as berat,sum(tmp_detail_kite.netto) as panjang,detail_pergerakan_stok.satuan FROM packing_list 
LEFT JOIN detail_pergerakan_stok ON detail_pergerakan_stok.refno = packing_list.listno
LEFT JOIN tmp_detail_kite ON tmp_detail_kite.id = detail_pergerakan_stok.id_detail_kj
LEFT JOIN tbl_kite ON tbl_kite.id = tmp_detail_kite.id_kite
WHERE tbl_kite.no_lot='$rowd[no_lot]' and tbl_kite.no_warna='$rowd[no_warna]' and tbl_kite.no_po='$po' and tbl_kite.nokk='$rowd[nokk]'
AND detail_pergerakan_stok.refno='$rowd[listno]'
GROUP BY detail_pergerakan_stok.satuan ");
 $jmldata1=sqlsrv_fetch_array($sqla1);
...
//kolom 1
 $sql1=sqlsrv_query($con,"SELECT * FROM packing_list 
LEFT JOIN detail_pergerakan_stok ON detail_pergerakan_stok.refno = packing_list.listno
LEFT JOIN tmp_detail_kite ON tmp_detail_kite.id = detail_pergerakan_stok.id_detail_kj
LEFT JOIN tbl_kite ON tbl_kite.id = tmp_detail_kite.id_kite
WHERE tbl_kite.no_lot='$rowd[no_lot]' and tbl_kite.no_warna='$rowd[no_warna]' and tbl_kite.no_po='$po' and tbl_kite.nokk='$rowd[nokk]' 
...
//kolom 2
 $sql2=sqlsrv_query($con,"SELECT * FROM packing_list 
LEFT JOIN detail_pergerakan_stok ON detail_pergerakan_stok.refno = packing_list.listno
LEFT JOIN tmp_detail_kite ON tmp_detail_kite.id = detail_pergerakan_stok.id_detail_kj
LEFT JOIN tbl_kite ON tbl_kite.id = tmp_detail_kite.id_kite
WHERE tbl_kite.no_lot='$rowd[no_lot]' and tbl_kite.no_warna='$rowd[no_warna]' and tbl_kite.no_po='$po' and tbl_kite.nokk='$rowd[nokk]'
...
//kolom 3
 $sql3=sqlsrv_query($con,"SELECT * FROM packing_list 
LEFT JOIN detail_pergerakan_stok ON detail_pergerakan_stok.refno = packing_list.listno
LEFT JOIN tmp_detail_kite ON tmp_detail_kite.id = detail_pergerakan_stok.id_detail_kj
LEFT JOIN tbl_kite ON tbl_kite.id = tmp_detail_kite.id_kite
WHERE tbl_kite.no_lot='$rowd[no_lot]' and tbl_kite.no_warna='$rowd[no_warna]' and tbl_kite.no_po='$po' and tbl_kite.nokk='$rowd[nokk]'
AND detail_pergerakan_stok.refno='$rowd[listno]'
order by detail_pergerakan_stok.id asc LIMIT $lakhr,$batas");
	if($rowd['no_item']!="")
...
<?php if($rowd['no_lot']!=''){echo $rowd['no_lot'];}?></td>
  ...
  <tr align="center"  class="tombol" >
    <td width="0" valign="top"><table width="100%" border="0">
      <tr align="center">
        <td width="15%" ><?php if($rowd['pack']=="ROLLS"){echo "ROLL NO";}else{echo "BALES NO";}?></td>
        <td>KGS</td>
      ...
      while($row1=sqlsrv_fetch_array($sql1))
  {
      ...
      <tr>
        ...
        <td align="right"><?php if($rowd['satuan']=="PCS"){echo number_format($row1['netto'],'0');}if($rowd['satuan']=="Yard" or $rowd['satuan']=="Meter"){echo  number_format($row1['yard_'],'2','.',',');}?></td>
        <td align="center"><?php if($row1['ukuran']!=""){echo str_replace(' ','',str_replace('x','*',$row1['ukuran']))."CM";}else{echo"-";}?></td>
        <td align="right"><?php if($rowd['pack']=="ROLLS"){echo number_format($row1['weight']+0.6,'2','.',',');}else{echo number_format($row1['weight']+0.2,'2','.',',');}?></td>
        <td align="right">-</td>
      </tr>
       <?php
      ...
      while($row2=sqlsrv_fetch_array($sql2))
  {
      ...
      <tr>
        ...
        <td align="right"><?php if($rowd['satuan']=="PCS"){echo number_format($row2['netto'],'0');}if($rowd['satuan']=="Yard" or $rowd['satuan']=="Meter"){echo  number_format($row2['yard_'],'2','.',',');}?></td>
        <td align="center"><?php if($row2['ukuran']!=""){echo str_replace(' ','',str_replace('x','*',$row2['ukuran']))."CM";}else{echo"-";}?></td>
        <td align="right"><?php if($rowd['pack']=="ROLLS"){echo number_format($row2['weight']+0.6,'2','.',',');}else{echo number_format($row2['weight']+0.2,'2','.',',');}?></td>
        <td align="right">-</td>
      </tr>
      <?php
      ...
      while($row3=sqlsrv_fetch_array($sql3))
  {
      ...
      <tr>
        ...
        <td align="right">
		<?php if($rowd['satuan']=="PCS"){echo number_format($row3['netto'],'0');}if($rowd['satuan']=="Yard" or $rowd['satuan']=="Meter"){echo  number_format($row3['yard_'],'2','.',',');}?></td>
        <td align="center"><?php if($row3['ukuran']!=""){echo str_replace(' ','',str_replace('x','*',$row3['ukuran']))."CM";}else{echo"-";}?></td>
        <td align="right"><?php if($rowd['pack']=="ROLLS"){echo number_format($row3['weight']+0.6,'2','.',',');}else{echo number_format($row3['weight']+0.2,'2','.',',');}?></td>
        <td align="right">-</td>
      </tr>
      <?php
  ...
  <tr class='tombol'>
    <td colspan="4" align="right"><strong><?php echo $foc; ?> </strong> &nbsp;Total 
      <?php if($rowd['no_lot']!=''){echo $rowd['no_lot'];}?></td>
    <td  align="right"><strong><?php echo $jmldata['roll'];?> <?php if($rowd['pack']=="ROLLS"){echo "Rolls";}else{echo "Bales";}?></strong></td>
    <td  align="right"><strong><?php echo number_format($jmldata['berat'],'2','.',',');?> Kg</strong></td>
    <td  align="right"><strong><?php 
	if ($jmldata['satuan']=="PCS"){echo number_format($jmldata1['panjang'],'0','.',',')." ".$jmldata1['satuan'];}
	if ($jmldata['satuan']=="Yard" Or $jmldata['satuan']=="Meter"){
	echo number_format($jmldata['panjang'],'2','.',',')." ".$jmldata['satuan'];}?></strong></td>
    </tr>
