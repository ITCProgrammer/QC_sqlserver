...
$format = date("y");
$sql=mysqli_query($con,"SELECT no_dok FROM pergerakan_stok WHERE substr(no_dok,1,2) like '%".$format."%' ORDER BY no_dok DESC LIMIT 1 ") or die (mysql_error());
...
and productprop.BatchNo='$_GET[kkno]' and   CAST(TM.dbo.stockmovement.[note] AS VARCHAR(8000)) $bgn like '%IM%'");	
	...
	*/
    ...
    <tr>
      <td colspan="6">
	  <?php if($_GET['kkno']!="" and $_GET['ke']=="GUDANG KAIN JADI"){$sqlC=mysqli_query($con,"select pergerakan_stok.no_mutasi,pergerakan_stok.userid,pergerakan_stok.tgl_update,count(detail_pergerakan_stok.weight) as rol,sum(detail_pergerakan_stok.weight) as berat from pergerakan_stok 
...
GROUP BY pergerakan_stok.id");?>Note:<br>
		<?php while($rC=mysqli_fetch_array($sqlC)){ echo "<font color=red>UserID: ".$rC['userid'].", No Mutasi: ".$rC['no_mutasi']." Jml Rol: ".$rC['rol'].", Qty: ".$rC['berat'].", Tgl Transfer Out: ".$rC['tgl_update']."</font><br>";} }?></td>
    </tr>
...
$qry=mysqli_query($con,"select * from tmp_detail_kite where nokkKite='".$_GET['kkno']."' and (sisa='TH' or sisa='FKTH' or sisa='BB') order by no_roll asc");}
 if($_GET['ke']=="GUDANG BS"){
	...
	{
		    	$qry=mysqli_query($con,"select * from tmp_detail_kite where nokkKite='".$_GET['kkno']."' and (sisa='BB' or sisa='BS' or sisa='STOCK') and ket='INSPEK' order by no_roll asc");
			}else{
				$qry=mysqli_query($con,"select * from tmp_detail_kite where nokkKite='".$_GET['kkno']."' and (sisa='BB' or sisa='BS' or sisa='STOCK') order by no_roll asc");}
 }
