...
ORDER BY tbl_kite.id ASC");
    ...
    <td align="right"><?php if($row8['ket']!="" and substr($_POST['user_name'],0,6)=="INSPEK"){$grab= number_format($row8['qty']-$row9['grd_c'],'2','.',',');}else{$grab= number_format($row2['grd_a_b']-$row6['grd_a_b'],'2','.',',');}
	echo $grab;
	?></td>
    <td align="right">
	
	<?php 
	$grc= number_format($row1['grd_c']-$row5['grd_c'],'2','.',','); echo $grc;?>
    ...
    <?php echo number_format($yrdmer,'2','.',',')." ".$row['satuan']; ?>
    ...
    <td align="center"><?php 
	if($row['no_mutasi']!='' and $row['user_packing']==$_POST['user_name']){$keterangan="OUT";}else{$keterangan="IN PROCESS";}
 ...
 <tr bgcolor="#99FFFF">
  ...
  <tr bgcolor="#99FFFF">
    ...
    <td>&nbsp;</td>
      ...
      <a href="pages/cetak/cetak.php?tgl1=<?php echo $tgl_cetak1; ?>&tgl2=<?php echo $tgl_cetak2; ?>&user_name=<?php echo $_POST['user_name']; ?>&sift=<?php echo $_POST['sift']; ?>" target="_blank">Cetak</a>                        
