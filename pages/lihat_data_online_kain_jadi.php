...
<body>
    <div id="art-main">
      <input type="button" class="art-button" onClick="window.location.href='../index_login.php?act=logout'" value="Logout"/>
                            <?php if($_SESSION['status']==0){ ?><input type="button" class="art-button" onClick="window.location.href='../index1.php'" value="Home"/><input type="button" class="art-button" onClick="window.location.href='../index1.php?p=laporan_harian_online_kain'" value="Laporan"/><br /><?php } ?>
                       
                        
       <table width="100%">
  <tr>
    <td>&nbsp;</td>
    <td colspan="26" align="center"><b>HARIAN ONLINE KAIN JADI </b></td>
  </tr>
  <tr>
   <?php $tgl_cetak1= $_POST['awal'];?>
    <td colspan="27"><b>Tanggal : <?php echo  date("d-M-Y", strtotime($tgl_cetak1))." s/d ".date("d-M-Y",strtotime ( '1 day' , strtotime ($tgl_cetak1))) ; 
...
and productprop.productiondate between '$tgl_cetak1 07:00:00' and '$tgl_cetak2 07:00:00' 
...
where stockmovement.wid=22 and stockmovement.transactiontype=4 and stockmovement.transactionstatus=1
and productprop.productiondate between '$tgl_cetak1 07:00:00' and '$tgl_cetak2 07:00:00' and productprop.BatchNo='$row[batchno]' and (processflowinspectionprocessno.PointGradeID='1' or processflowinspectionprocessno.PointGradeID='2')");
...
left join  processflowinspectionprocessno on processflowinspectionprocessno.id=productprop.pfipnid
where stockmovement.wid=22 and stockmovement.transactiontype=4 and stockmovement.transactionstatus=1
and productprop.productiondate between '$tgl_cetak1 07:00:00' and '$tgl_cetak2 07:00:00' and productprop.BatchNo='$row[batchno]' and  processflowinspectionprocessno.PointGradeID='3'");
...
left join  processflowinspectionprocessno on processflowinspectionprocessno.id=productprop.pfipnid
where stockmovement.wid=22 and stockmovement.transactiontype=4 and stockmovement.transactionstatus=1
and productprop.productiondate between '$tgl_cetak1 07:00:00' and '$tgl_cetak2 07:00:00' and productprop.BatchNo='$row[batchno]' and (stockmovementdetails.RefNo like'%sisa%' or productprop.rollno like'%sisa%')
");
...
and productprop.productiondate between '$tgl_cetak1 07:00:00' and '$tgl_cetak2 07:00:00' and productprop.BatchNo='$row[batchno]' and stockmovementdetails.RefNo like'%sisa%'
...
left join stockmovementdetails on  stockmovementdetails.stockmovementid=stockmovement.id
left join productprop on productprop.id = stockmovementdetails.productpropid
where stockmovement.wid=22 and stockmovement.transactiontype=4 and stockmovement.transactionstatus=1 and productprop.productiondate between '$tgl_cetak1 07:00:00' and '$tgl_cetak2 07:00:00' and (stockmovementdetails.refno like '%extra%' or stockmovementdetails.refno like '%FOC%') and productprop.BatchNo='$row[batchno]'");
    ...
    <tr bgcolor="#9999FF">
    <td><?php echo $item; ?></td>
   <?php $langgan = $r5['partnername']." / ".$r6['partnername']; ?> 
    ...
    <td><a href="../index1.php?p=produksi_detail_online&id=<?php echo $row['batchno'];?>&tgl1=<?php echo $tgl_cetak1; ?>&tgl2=<?php echo $tgl_cetak2; ?>&langgan=<?php echo $langgan;?>"><?php echo $row['batchno']; ?></a></td>
  ...
  <tr bgcolor="#99FFFF">
    ...
    <a href="cetak_data_online_kain.php?tgl1=<?php echo $tgl_cetak1; ?>&tgl2=<?php echo $tgl_cetak2; ?>&user_name=<?php echo $_POST['user_name']; ?>&sift=<?php echo $_POST['sift']; ?>">Cetak</a>                        
