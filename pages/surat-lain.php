  ...
  <table width="100%" border="0">
    ...
    <tr>
      ...
      <td><input name="awal" type="text" id="awal" value="<?php echo $_GET['tglkirim'];?>" onchange="window.location='?p=surat-lain&amp;no_sj=<?php echo $_GET['no_sj'];?>&amp;tglkirim='+this.value"  placeholder="0000-00-00" pattern="[0-9]{4}-[0-9]{2}-[0-9]{2}$"  required="required" size="10"/>
        *</td>
      <?php 
			$sql=sqlsrv_query($con,"select * from tbl_pengiriman where no_sj='".$_GET['no_sj']."' and tgl_kirim='".$_GET['tglkirim']."' and kategori='lain-lain' order by id asc");
...
<table width="100%"  border="0">
  ...
  <?php 
			$qry=sqlsrv_query($con,"select * from tbl_pengiriman where no_sj='".$_GET['no_sj']."' and tgl_kirim='".$_GET['tglkirim']."' and tmp_hapus='0' and kategori='lain-lain' order by id asc");
			$no=1;
