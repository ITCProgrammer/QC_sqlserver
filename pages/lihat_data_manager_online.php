...
<body>
    <div id="art-main">
      <input type="button" class="art-button" onClick="window.location.href='../index_login.php?act=logout'" value="Logout"/>
                            <?php if($_SESSION['status']==0){ ?><input type="button" class="art-button" onClick="window.location.href='../index1.php'" value="Home"/><input type="button" class="art-button" onClick="window.location.href='../index1.php?p=laporan_harian_manager_online'" value="Laporan"/><br /><?php } ?>
                       
                        
       <table width="100%">
  <tr>
    <td>&nbsp;</td>
    <td colspan="22" align="center"><b>MUTASI KAIN ONLINE <?php echo " ".$_POST['bagian'];?></b></td>
  </tr>
  <tr>
   <?php $tgl_cetak1= $_POST['awal'];
   if($_POST['sift']=="3"){$tgl_cetak2=date("Y-m-d",strtotime ( '1 day' , strtotime ($tgl_cetak1)));}else{$tgl_cetak2= $_POST['awal'];} ?><td colspan="23"><b>Tanggal : <?php echo  date("d-M-Y", strtotime($tgl_cetak1))." s/d ".date("d-M-Y", strtotime($tgl_cetak2)); ?><br> SHIFT : <?php echo $_POST['sift'];?></b></td>
  </tr>
  ...
  <?php 
  if($_POST['bagian']=="PACKING"){$bgn="not";}else{$bgn=" ";}  
  if($_POST['sift']=="1"){
...
group by stockmovement.ID,stockmovement.PONo,productprop.BatchNo,productprop.PONo,productprop.Width,productprop.WeightPerArea");  
  ...
  while($row=mssql_fetch_array($sqlsvr1))
  {
	  ...
	  if($crow>0){$keterangan="OUT";}else{$keterangan="IN PROCESS";}
...
left join  processflowinspectionprocessno on processflowinspectionprocessno.id=productprop.pfipnid
where stockmovement.wid=22 and stockmovement.transactiontype=4 and stockmovement.transactionstatus=1
and productprop.productiondate between '$tgl_cetak1 15:00:00' and '$tgl_cetak2 22:59:59' and productprop.BatchNo='$row[batchno]' and (stockmovementdetails.RefNo like'%sisa%' or productprop.rollno like'%sisa%')
");
$row3=mssql_fetch_array($sqlsvr4);
...
left join  processflowinspectionprocessno on processflowinspectionprocessno.id=productprop.pfipnid
where stockmovement.wid=22 and stockmovement.transactiontype=4 and stockmovement.transactionstatus=1
and productprop.productiondate between '$tgl_cetak1 23:00:00' and '$tgl_cetak2 06:59:59' and productprop.BatchNo='$row[batchno]' and (stockmovementdetails.RefNo like'%sisa%' or productprop.rollno like'%sisa%')
");
$row3=mssql_fetch_array($sqlsvr4);
...
where wid='12' and transactiontype='7' and processcontrolbatches.documentno='$row[batchno]'");
    ...
    <td>
	 <a href="../index1.php?p=mutasi_detail_online&id=<?php echo $row['batchno'];?>&tgl1=<?php echo $tgl_cetak1; ?>&tgl2=<?php echo $tgl_cetak2; ?>&bagian=<?php echo $_POST['bagian'];?>&sift=<?php echo $_POST['sift'];?>"><?php echo $row['batchno']; ?></a>
