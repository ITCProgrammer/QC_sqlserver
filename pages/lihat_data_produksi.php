    ...
    <div id="art-main">
      <div class="art-Sheet-bl"></div>
            ...
            <input type="button" class="art-button" onClick="window.location.href='../index_login.php?act=logout'" value="Logout"/>
                            <?php if($_SESSION['status']==0){ ?><input type="button" class="art-button" onClick="window.location.href='../index1.php'" value="Home"/><input type="button" class="art-button" onClick="window.location.href='../index1.php?p=laporan_harian_produksi'" value="Laporan"/><br /><?php } ?>
...
GROUP BY tbl_kite.id,detail_kite.sisa,detail_kite.nokkKite
...
$sql7=mysql_query("SELECT *, sum(detail_kite.net_wight) as qty,count(detail_kite.sisa) as jml , grade
FROM tbl_kite
LEFT JOIN detail_kite ON detail_kite.nokkKite = tbl_kite.nokk
WHERE sisa = 'FOC'
AND nokkKite = '$row[nokk]'
...
ORDER BY tbl_kite.id ASC");
 ...
 $sql8=mysql_query("SELECT *, sum(detail_kite.net_wight) as qty,count(detail_kite.ket) as jml , sum(detail_kite.yard_) as yrd12, grade
FROM tbl_kite
LEFT JOIN detail_kite ON detail_kite.nokkKite = tbl_kite.nokk
WHERE ket='INSPEK'
AND nokkKite = '$row[nokk]'
...
ORDER BY tbl_kite.id ASC");
 $row8=mysql_fetch_array($sql8);
    ...
    <td align="right"><?php if($row['sisa']=="SISA"){echo $row4['jml'];}else if($row8['ket']!=""){echo $row8['jml'];}else{echo $row['roll'];}?></td>
    <td align="right">
	<?php if($row['sisa']=="SISA"){
		if($row4['grade']=="A" || $row4['grade']=="B"){
		echo number_format($row4['qty'],'2','.',',');}else{echo number_format($row2['grd_a_b'],'2','.',',');}}else if($row['ket']!=''){echo number_format($row8['qty'],'2','.',',');}else if($row['sisa']=="FOC"){}else if($row8['ket']!=""){echo $row8['qty'];}else
		if($row4['grade']=="A" || $row4['grade']=="B"){echo number_format($row2['grd_a_b']-$row6['grd_a_b'],'2','.',',');}else{echo number_format($row2['grd_a_b'],'2','.',',');}
		?>
	</td>
    <td align="right">
	<?php if($row['sisa']=="SISA"){echo number_format($row5['grd_c'],'2','.',',');}else{echo number_format($row1['grd_c'],'2','.',',');} ?></td>
    <td><?php if($row['sisa']=="SISA"){echo $row['sisa'];}else if($row['sisa']=="FKSI"){echo"SISA";}else if($row['sisa']=="FOC"){echo"EXTRA";}?></td>
    <td align="right"><?php if($row8['ket']!=""){echo $row8['yrd12']." ".$row['satuan'];}else{echo number_format($row['yard_'],'2','.',',')." ".$row['satuan'];}?></td>
    <td><?php echo $row['tempat']; ?></td>
    <td><?php if($row['sisa']=="FOC"){ echo number_format($row7['qty'],'2','.',',');}?></td>
    <td><?php echo $row['lebar'];?></td>
    <td>X</td>
    <td><?PHP echo $row['berat']; ?></td>
    <td>&nbsp;</td>
    <td align="center"><?php 
	if($row['no_mutasi']!=''){$keterangan="OUT";}else{$keterangan="IN PROCESS";}
	
	echo "<font color='#FF0000'>".$keterangan."</font>";?></td>
  </tr>
