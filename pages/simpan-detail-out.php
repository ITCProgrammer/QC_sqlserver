...
<table width="70%" border="0" class="table-list1">
  ...
  $sql2=mysqli_query($con,"Select * from `db_qc`.`tbl_kite` 
...
$nopo=addslashes($_POST['no_po']);
   $datastk=mysqli_query($con,"insert into `pergerakan_stok`(`typetrans`,`typestatus`,`no_dok`,`no_order`,`no_po`,`ket`,`fromtoid`,`tgl_update`,`userid`) values('1','1','".$_POST['no_dok']."','".$_POST['no_do']."','$nopo','".$_POST['catatan']."','".$_POST['partners']."',NOW(),'".$_POST['user_name']."')")or die("Gagal11");
   $sql3=mysqli_query($con,"Select `id` from `pergerakan_stok` where  `pergerakan_stok`.`no_dok`='".$_POST['no_dok']."' ");
$row3=mysqli_fetch_array($sql3);
 if($_POST['partners']=="INSPEK MEJA"){
$sql1=mysqli_query($con,"select * from tmp_detail_kite where nokkKite='".$_POST['nokk']."' and (sisa='TH' or sisa='FKTH' or sisa='BB')");}
 if($_POST['partners']=="GUDANG BS"){
$sql1=mysqli_query($con,"select * from tmp_detail_kite where nokkKite='".$_POST['nokk']."' and (sisa='BB' or sisa='BS' or sisa='STOCK') ");}
if($_POST['partners']=="GUDANG KAIN JADI" or $_POST['partners']=="GUDANG GREIGE"){
	$sql1=mysqli_query($con,"select * from tmp_detail_kite where nokkKite='".$_POST['nokk']."' and (sisa!='TH' and sisa!='FKTH' and sisa!='BS' and sisa!='BB')");
	}
  $n=1;
  $nom=1;
 ...
 if($_POST['check'][$n]!='')
		 ...
		 $id_kite=$_POST['check'][$n];
			  $sqldtl=mysqli_query($con,"SELECT `tmp_detail_kite`.`rolexp`,`tmp_detail_kite`.`id_kite` as kd_kite,`tmp_detail_kite`.`id` as kd,`tmp_detail_kite`.`nokkKite`,`tmp_detail_kite`.`ket_c`,
...
mysqli_query($con,"insert into `detail_pergerakan_stok` (`id_stok`,`id_detail_kj`,`weight`,`yard_`,`status`,`no_roll`,`grade`,`satuan`,`sisa`,`ket`,`nokk`,`ket_c`,`SN`,`weight_basic`,`yard_basic`) values('".$row3['id']."','".$rdtkite['kd']."','".$rdtkite['net_wight']."','".$rdtkite['yard_']."','0','".$rdtkite['no_roll']."','".$rdtkite['grade']."','".$rdtkite['satuan']."','".$rdtkite['sisa']."','".$rdtkite['ket']."','".$rdtkite['nokkKite']."','$ketc','".$rdtkite['SN']."','".$rdtkite['net_wight']."','".$rdtkite['yard_']."')")or die("Gagal1  $id_kite");
  ...
  if($datastk)
  {
	  echo "<script>alert('Data Tersimpan');window.location.href='../index1.php?p=transfer-out&kkno=".$_POST['nokk']."&ke=".$_POST['partners']."&tes=".$rowsvr['ID']." ".$rowsvr1['InspectionNo']."';</script>";
		 ...
		 $qry=mysqli_query($con,"select pergerakan_stok.id,bruto,satuan,
...
while($row=mysqli_fetch_array($qry))
	  {
		if($_POST['check'][$n]!='')
		 ...
		 $id_kite=$_POST['check'][$n];
			 $u1=mysqli_query($con,"Update pergerakan_stok set tgl_update=now(),fromtoid='GUDANG KAIN JADI',userid='".$_POST['user_name']."' where id='".$row['id']."'")or die("GAGAL Update 1");
			  $u2=mysqli_query($con,"Update detail_pergerakan_stok set status='0' where id='$id_kite'")or die("GAGAL Update 1");
