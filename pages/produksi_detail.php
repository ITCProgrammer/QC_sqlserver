...
ORDER BY detail_kite.no_roll ASC");
...
WHERE tbl_kite.nokk = '$_GET[id]'
ORDER BY detail_kite.no_roll ASC limit 0,20"); 
...
INNER JOIN detail_kite ON detail_kite.nokkKite = tbl_kite.nokk
WHERE tbl_kite.nokk = '$_GET[id]'
ORDER BY detail_kite.no_roll ASC limit 20,20"); 
...
INNER JOIN detail_kite ON detail_kite.nokkKite = tbl_kite.nokk
WHERE tbl_kite.nokk = '$_GET[id]'
ORDER BY detail_kite.no_roll ASC limit 40,20");
  ...
  {	
    ...
    <td align="right"><?php echo number_format($row['yard_'],'2','.',',');?></td>
     <td><?php echo $row['sisa'];?></td>
      ...
      <?php
	   if($row['sisa']=="SISA" ||$row['sisa']=="FOC"){$netto=0; $ukuran=0;}else{$netto=$row['bruto']; $ukuran=$row['yard_'];}
