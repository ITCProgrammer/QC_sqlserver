...
$con=mysqli_connect("10.0.0.10","dit","4dm1n","db_qc");	
		...
		$format = date("y")."1";
		$sql=mysqli_query($con,"SELECT refno FROM pergerakan_stok WHERE substr(refno,1,3) like '%".$format."%'
...
if(isset($_POST['btnBatal'])){
	# Kosongkan Tmp jika datanya sudah dipindah
		$hapusSql = "DELETE FROM tmp_detail_pergerakan_stok WHERE transtatus='1' AND userid='".$usernm."'";
...
if(isset($_POST['btnTambah'])){
		...
		$cekSql ="SELECT * FROM tmp_detail_pergerakan_stok WHERE SN='$Barcode' AND transtatus='1' AND userid='".$usernm."'"; 
		...
		$cekSql3 ="SELECT * FROM detail_pergerakan_stok WHERE SN='$Barcode' AND sisa='BS'"; 
		$cekQry3 = mysqli_query($con,$cekSql3) or die ("Gagal Query d");
		if ((mysqli_num_rows($cekQry) >= 1) or (mysqli_num_rows($cekQry1) >= 1) or (mysqli_num_rows($cekQry2)>=1) or (mysqli_num_rows($cekQry3)>=1) or (mysqli_num_rows($cekQry0)<=0)) {
			$pesanError = array();
			if(mysqli_num_rows($cekQry1)>=1){
			...
			$myQry = mysqli_query($con,$mySql) or die ("Gagal Query Tmp 1 ");
			...
			if ($myQty >= 1) {
				...
				mysqli_query($con,$tmpSql) or die ("Gagal Query tmp 2 : ");
			}}
	}
...
if(isset($_POST['btnSimpan'])){
	...
	$tmpSql ="SELECT COUNT(*) As qty FROM tmp_detail_pergerakan_stok WHERE transtatus='1' AND userid='".$usernm."'";
	$tmpQry = mysqli_query($con,$tmpSql) or die ("Gagal Query Tmp 3");
	...
	if (count($pesanError)>=1 ){
		...
		$mySql	= "INSERT INTO pergerakan_stok SET 
						...
						refno ='$txtDok',
						userid='".$usernm."'";
		mysqli_query($con,$mySql) or die ("Gagal query 4 ");
		...
		$tmpQry = mysqli_query($con,$tmpSql) or die ("Gagal Query Tmp 5");
		...
		$qrySts=mysqli_query($con,"SELECT count(*) FROM tbl_ncp_qcf WHERE `status`='Belum OK' and nokk='$dataKK'");
		$StsCk=mysqli_num_rows($qrySts);
		if ($StsCk>0) {
		...
		# Kosongkan Tmp jika datanya sudah dipindah
		$hapusSql = "DELETE FROM tmp_detail_pergerakan_stok WHERE transtatus='1' AND userid='".$usernm."'";
		mysqli_query($con,$hapusSql) or die ("Gagal kosongkan tmp 6");		
		// Refresh form
		//echo "<meta http-equiv='refresh' content='0; url=cetak/transaksi_pembelian_cetak.php?noNota=$noTransaksi'>";
		echo "<script>";
		//echo "window.open('../cetak/benang_masuk_cetak.php?noNota=$noTransaksi', width=330,height=330,left=100, top=25)";
...
<fieldset>
  ...
  <tr>
    ...
    <td><select name="terima" id="terima" onchange="window.location='index1.php?p=kain_masuk_barcode&terima='+this.value">
      ...
      <option value="PACKING"  <?php if ('PACKING'== $DataTerima) {echo "selected";}else{ if($_GET['terima']=="PACKING"){echo "selected";}else{echo "";}}?>>PACKING</option>
      <option value="INSPEK MEJA" <?php if ('INSPEK MEJA'== $DataTerima) {echo "selected";}else{ if($_GET['terima']=="INSPEK MEJA"){echo "selected";}else{echo "";}}?>>INSPEK MEJA</option>
      <option value="GANTI STIKER" <?php if ('GANTI STIKER'== $DataTerima) {echo "selected";}else{ if($_GET['terima']=="GANTI STIKER"){echo "selected";}else{echo "";}}?> >GANTI STIKER</option>
	  <option value="REVISI STIKER" <?php if ('REVISI STIKER'== $DataTerima) {echo "selected";}else{ if($_GET['terima']=="REVISI STIKER"){echo "selected";}else{echo "";}}?> >REVISI STIKER</option>	
      <option value="POTONG SISA" <?php if ('POTONG SISA'== $DataTerima) {echo "selected";}else{ if($_GET['terima']=="POTONG SISA"){echo "selected";}else{echo "";}}?> >POTONG SISA</option>
      <option value="QC BS" <?php if ('QC BS'== $DataTerima) {echo "selected";}else{ if($_GET['terima']=="QC BS"){echo "selected";}} ?>>QC BS</option>
      <option value="LAIN" <?php if ('LAIN'== $DataTerima) {echo "selected";}else{ if($_GET['terima']=="LAIN"){echo "selected";}else{echo "";}}?>>LAIN-LAIN</option>
    </select>
    </td>
    </tr>
    <tr>
  ...
  <tr>
    ...
    <td><label for="blok"></label>
      <input name="blok" type="text" id="blok" size="15"  r/></td>
  </tr>
  <?Php } ?>
  <tr>
    <td>Shift</td>
...
<fieldset>
  ...
  <tr>
    ...
    <td><select name="lokasi" id="lokasi">
	  ...
	  <?php $qryLok=mysqli_query($con,"SELECT lokasi FROM tbl_lokasi ORDER BY lokasi ASC");
        ...
        <option value="<?php echo $rLok['lokasi']; ?>" <?php if($Lokasi==$rLok['lokasi']){ echo "SELECTED"; }?>><?php echo $rLok['lokasi']; ?></option>
		<?php } ?>
  ...
  <table width="" border="0" cellpadding="1" cellspacing="1">
    ...
    <?php
	//tambahan
	$data=mysqli_query($con,"SELECT a.*,b.pelanggan,b.no_order,b.no_po,b.no_lot,b.warna FROM tmp_detail_pergerakan_stok a
	INNER JOIN tmp_detail_kite c on c.id=a.id_detail_kj
INNER JOIN tbl_kite b ON c.id_kite = b.id WHERE transtatus='1' and a.userid='".$usernm."' order by a.no_roll ASC");
	$no=1;
