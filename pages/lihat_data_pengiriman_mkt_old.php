...
while($row=mysql_fetch_array($sql)){
	...
	$cek=mysql_query("SELECT id_list from tbl_pengiriman where id_list='$row[id]' and nokk='$row[nokk]' and (ukuran='$ukuran'or idtmp='$row[idtmp]') limit 1");
...
while($row3=mysql_fetch_array($sqlbr)){
    $sqlmkt=mssql_query("SELECT a.DocumentNo,c.Color,d.PONumber,e.ProductCode,b.UnitPrice,f.UnitName,g.Symbol from JobOrders a
    ...
    $rowmkt=mssql_fetch_array($sqlmkt);

    if($_POST['awal']!="" AND $row3['satuan_mkt']==''){
        $sqlupdate=mysql_query("UPDATE tbl_pengiriman SET 
