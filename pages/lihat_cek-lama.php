...
<body>
  ...
  <tr>
    <td colspan="28" align="center"><b>CEK LAPORAN HARIAN KAIN JADI PER MUTASI</b></td>
    </tr>
  <tr>
   <?php $tgl_cetak1=$_POST['awal'];?>
    
    <td colspan="28"><b>No Mutasi : <?php echo $_GET['mutasi'];?>   
      </b>
      <?php 
...
GROUP BY no_mutasi");
  ...
  <tr align="center" bgcolor="#0099FF" class="ukuran">
    ...
    <td rowspan="2" width="15">JENIS KAIN</td>
    ...
    <td colspan="3">QC</td>
    <td colspan="4">MASUK KAIN JADI</td>
    <td colspan="4">KELUAR KAIN JADI</td>
...
$rtmpt=mysql_fetch_array($stmpt);
	$sqlket=mysql_query("select nokk,ket_c,sisa from detail_pergerakan_stok where nokk ='$row[nokk]' and ket_c !='' and sisa !='TH' and sisa !='FKTH' and grade='C'
GROUP BY ket_c");
$rowket=mysql_fetch_array($sqlket);
// query masuk gudang
$sqlmskg=mysql_query("SELECT
	a.id,b.nokk,b.sisa,
  ...
  SUM(case when b.grade='A' or b.grade='B' or b.grade='C' or b.grade='' then b.yard_ else 0 end) as tot_yard,
	...
	WHERE
	b.nokk='$row[nokk]' and b.status='1' and b.sisa='$row[sisa]'
	GROUP BY
	b.nokk,b.sisa
	ORDER BY
	a.id ");
	$rmskg=mysql_fetch_array($sqlmskg);
	// query keluar gudang
	$sqlklrg=mysql_query("SELECT
	a.id,b.nokk,b.sisa,
  SUM(case when b.grade='A' or b.grade='B' or b.grade='C' or b.grade='' then b.weight else 0 end) as tot_qty,
  SUM(if(b.grade='A' or b.grade='B' or b.grade='C' or b.grade='', 1, 0)) as tot_rol,
  SUM(case when b.grade='A' or b.grade='B' or b.grade='C' or b.grade='' then b.yard_ else 0 end) as tot_yard,
	...
	LEFT JOIN tbl_kite c ON c.id = d.id_kite
	WHERE
	b.nokk='$row[nokk]' and b.status='2' and b.sisa='$row[sisa]'
	GROUP BY
	b.nokk,b.sisa
	ORDER BY
	a.id ");
	$rklrg=mysql_fetch_array($sqlklrg);
	  ?>
    ...
    <td bgcolor="#CCCC33"><a href="#"onClick="window.open('detail-masuk-kain-m.php?nokk=<?php echo $row['nokk'];?>&ket=<?php echo $row['sisa'];?>&id=<?php echo $row['id'];?>','MyWindow','height=400,width=500');"><?php 
	$rol=$row['tot_rol'];
	echo $rol;
	?></a></td>
    <td align="right" bgcolor="#CCCC33"><?php 
	echo number_format($row['grd_ab'],'2','.',',');
	?></td>
    <td align="right" bgcolor="#CCCC33"><?php 
	...
	echo number_format($rmskg['grd_ab'],'2','.',',');
	?></td>
    <td bgcolor="#CCCC66"><?php 
	...
	echo number_format($rklrg['grd_ab'],'2','.',',');
	?></td>
    <td bgcolor="#FF6666"><?php 
