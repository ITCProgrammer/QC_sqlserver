...
<table width="100%" border="0" class="table-list1">
  <tr>
    ...
    <td width="49%" valign="top"><strong>Kepada Yth.<br />
      <?php 
	$sql=mssql_query("select * from partners where partnername like '%APPAREL ONE%'"); 
...
echo $nama2;
//echo $data['PartnerName']; ?>
      <?php
$sql2=mssql_query("select * from partners where partnername like '%APPAREL ONE%'");
...
if($data[ID]!=$data2[ID]){
	if($rAlt['alamat1']!=""){$kirim=" Kirim Ke: ".$rAlt['alamat1']." ";}else{$kirim="Kirim Ke : ".$data['PartnerName']." ";}
	}
...
$sqlitm1=mssql_query("SELECT description from ProductMaster where ProductNumber='".$itemno."-".$nowarna."'");
 ...
 $rowitm2=mssql_fetch_array($sqlitmp1);
	   ...
	   echo $rowitm['productcode']."/". htmlentities($rjns['description'],ENT_QUOTES); } 
		   ...
		   echo $itemno."/". htmlentities($jenis_kain,ENT_QUOTES);
		   } 
	   else if($itemno==""){
