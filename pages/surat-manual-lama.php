...
<form id="form1" name="form1" method="post" action="">
  <table width="100%" border="0">
    <tr>
      ...
      <td>:</td>
      <td><input name="no_sj" type="text" id="no_sj" size="7"  onchange="window.location='?p=surat-manual&amp;no_sj='+this.value"  value="<?php echo $_GET['no_sj'];?>" required="required"/>
        *</td>
      <td width="7%">&nbsp;</td>
      <td width="1%">&nbsp;</td>
      <td width="14%">&nbsp;</td>
    ...
    <tr>
      ...
      <td><input name="awal" type="text" id="awal" value="<?php echo $_GET['tglkirim'];?>" onchange="window.location='?p=surat-manual&amp;no_sj=<?php echo $_GET['no_sj'];?>&amp;tglkirim='+this.value"  placeholder="0000-00-00" pattern="[0-9]{4}-[0-9]{2}-[0-9]{2}$"  required="required" size="10"/>
        *</td>
      <?php 
			$sql=mysql_query("select * from tbl_pengiriman where no_sj='$_GET[no_sj]' and tgl_kirim='$_GET[tglkirim]' order by id asc");
    ...
    <tr>
      ...
      <td>Qty</td>
      <td>:</td>
      <td colspan="3"><input name="qty" type="text" id="qty" size="6" pattern="[0-9.]{1,}" placeholder="0.00" required /></td>
    </tr>
    <tr>
      <td>No. Order</td>
      <td>:</td>
...
<table width="100%"  border="0">
  <tr align="center" bgcolor="#66FF99">
    ...
    <td width="51%">JENIS KAIN</td>
    <td width="4%">LOT</td>
    <td width="4%">FOC</td>
    <td width="6%">AKSI</td>
  </tr>
  <?php 
			$qry=mysql_query("select * from tbl_pengiriman where no_sj='$_GET[no_sj]' and tgl_kirim='$_GET[tglkirim]' and tmp_hapus='0' order by id asc");
			$no=1;
  ...
  <tr bgcolor="<?php echo $bgcolor;?>" >
    ...
    <td align="center"><a href="?p=surat-manual&id=<?php echo $row['id'];?>&h=1&sj=<?php echo $_GET['no_sj'];?>">HAPUS</a></td>
  </tr>
  <?php $no++;}?>
</table>
</body>
...
if($_POST['SIMPAN']=="SIMPAN"){
	...
	$qrysimpan=mysql_query("INSERT INTO `tbl_pengiriman` (`no_sj`, `warna`, `rol`, `qty`, `buyer`, `no_po`, `no_order`, `jenis_kain`, `lot`, `tujuan`, `ket`, `tgl_kirim`, `tgl_buat`, `tgl_update`,`tmp_hapus`,`foc`) VALUES ('$_POST[no_sj]', '$_POST[warna]', '$_POST[rol]', '$_POST[qty]', '$_POST[pelanggan]', '$po', '$_POST[no_order]', '$jk', '$_POST[lot]', '$_POST[tujuan]', '$_POST[ket]', '$_POST[awal]', '$_POST[buat]', now(),'0','$_POST[foc]')");
	if($qrysimpan){
		echo "<script> alert('Data Tersimpan');window.location.href='?p=surat-manual&no_sj=$_POST[no_sj]'</script>";
		}
	}
if($_GET['h']=="1"){
	$qryhapus=mysql_query("UPDATE tbl_pengiriman SET id_list=null,`tmp_hapus`='1' WHERE id=$_GET[id]");
	if($qryhapus){
		echo "<script> alert('Data Telah dihapus');window.location.href='?p=surat-manual&no_sj=$_GET[sj]'</script>";}
	}	
?>