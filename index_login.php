...
if ($_POST) { //login user
  ...
  $level    =  $_POST['level'];
  $sql = "SELECT DISTINCT * FROM db_qc.user_login ul WHERE ul.[user]='$username' and password='$password' and level = '$level' ";
  $sql = sqlsrv_query($con, $sql, $dt, $options);
  if ($sql === false) {
  ...
  if (sqlsrv_num_rows($sql) > 0) {
    ...
    echo "<script>alert('Login Gagal!! $username');window.location='index_login.php';</script>";
  }
} else
